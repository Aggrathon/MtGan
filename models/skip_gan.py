...
class SkipGanGenerator(Generator):
    ...
    def __init__(self, training=True):
        ...
        with self.scope:
            ...
            self.fake_image = _generator(self.seed, False, training)
            self.real_disc = _discriminator(self.real_image, False, training)
            ...
            self.loss_d = tf.losses.compute_weighted_loss(
                [tf.reduce_mean(self.fake_disc), tf.reduce_mean(self.real_disc), self.gradient_penalty],
                [1.0, -1.0, 10.0]
            )
            ...
            self.trainer_g = tf.group(trainer_g, measure)
