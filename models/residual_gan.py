...
class ResidualGenerator(Generator):
    ...
    def __init__(self, training=True):
        ...
        with self.scope:
            ...
            self.gradient_penalty = tf.reduce_mean(tf.square(slope-2.0), name='gradient_penalty')
            #losses
            rmf = tf.reduce_mean(self.fake_critic)
            rmr = tf.reduce_mean(self.real_critic)
            diff = rmr - rmf
            center = tf.reduce_mean(tf.square(1.0-self.real_critic))
            ...
            #training
            gen_lr = LEARNING_RATE * 0.5 * tf.minimum(3.0, tf.maximum(0.01, distance))
            with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
                trainer_d = tf.train.AdamOptimizer(LEARNING_RATE*2, 0, 0.9, name='AdamD').minimize(
                    self.loss_d,
                    var_list=disc_vars,
                    name='train_d'
                )
                trainer_g = tf.train.AdamOptimizer(gen_lr, 0, 0.9, name='AdamG').minimize(
                    self.loss_g,
                    global_step=self.global_step,
                    var_list=gen_vars,
                    name='train_g'
                )
            self.trainer = tf.group(trainer_d, trainer_g, distance)
            #summary
    ...
    def train_step(self, summary=False, session=None):
        ...
        if summary:
            _, smry, step, res = session.run([self.trainer, self.summary, self.global_step, self.distance])
            self.add_summary(smry, step)
        else:
            _, step, res = session.run([self.trainer, self.global_step, self.distance])
        return step, res
